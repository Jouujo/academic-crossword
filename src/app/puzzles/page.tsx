                    ...
                    {filtered.map((p) => (
                        <div key={p.id} className="border rounded-md p-4 flex items-center justify-between">
                            <div>
                                ...
                                {p.tags && p.tags.length > 0 && (
                                    <div className="mt-1 flex flex-wrap gap-1">
                                        {p.tags.map((t) => (
                                            <span key={t} className="text-[10px] bg-gray-100 rounded px-1 py-0.5">{t}</span>
                                        ))}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex gap-2 text-sm">
                                <a className="px-3 py-1 rounded-md bg-gray-100 hover:bg-gray-200" href={`/student?id=${encodeURIComponent(p.id)}`}>Play</a>
                            </div>
                        </div>
                    ))}
                </div>
